...
from .query_neon import query_iitm_course_knowledge_db as _query_iitm_course_knowledge_db
...
def search_handbook_policy(
    ...
    return _query_pdf(
        query=query,
        pdf_path="student_handbook",
        model=model
    )


def search_grading_policy(
    query: str,
    model: Optional[str] = None
) -> PDFQueryResult:
...
def query_course_database(
    ...
    """
    return _query_iitm_course_knowledge_db(
        query=query,
        params=params
    )
