...
- smart_query(query, program="ds", level="foundation", n_results={DEFAULT_TOP_K}, score_threshold={THRESHOLD_HINT}): primary tool for querying the chunked ChromaDB data. It intelligently searches across relevant collections for the DS foundation level.
- query_by_program_and_level("ds", "foundation", query, n_results={DEFAULT_TOP_K}): Alternative tool for specific program/level queries.
...
Query strategy:
- Use smart_query("ds", "foundation", <keywords>, n_results={DEFAULT_TOP_K}) as your primary tool
...
- The tool will automatically search across all relevant DS foundation level collections and you can narrow noisy hits using score_threshold when appropriate.
