...
For context about the diploma level of IITM BS Degree Program, use the enhanced tools provided to you.

The tools you have are:
- smart_query(query, program="ds", level="diploma", n_results={DEFAULT_TOP_K}, score_threshold={THRESHOLD_HINT}): primary tool for querying the chunked ChromaDB data. It intelligently searches across relevant collections for the DS diploma level.
- query_by_program_and_level("ds", "diploma", query, n_results={DEFAULT_TOP_K}): Alternative tool for specific program/level queries.
...
Query strategy:
- Use smart_query("ds", "diploma", <keywords>, n_results={DEFAULT_TOP_K}) as your primary tool
...
- The tool will automatically search across all relevant DS diploma level collections and you can apply score_threshold to trim irrelevant hits.

After gathering information, provide a comprehensive and accurate answer based on the retrieved data.
Include relevant details about both Diploma in Programming and Diploma in Data Science components.
"""
